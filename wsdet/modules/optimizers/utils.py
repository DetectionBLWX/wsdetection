...
def adjustLearningRate(optimizer, optimizer_cfg=None):
    ...
    elif policy_cfg['type'] == 'step':
        target_lr = selected_optim_cfg['learning_rate']
        num_epochs, adjust_epochs, scale_factor = policy_cfg['opts']['num_epochs'], policy_cfg['opts']['adjust_epochs'], policy_cfg['opts']['scale_factor']
        for epoch in range(num_epochs):
            if epoch in adjust_epochs: target_lr = target_lr * scale_factor
        for param_group in optimizer.param_groups:
            if ('params_rules' in optimizer_cfg) and (optimizer_cfg['params_rules']):
                param_group['lr'] = target_lr * optimizer_cfg['params_rules'][param_group['name']]
            else:
